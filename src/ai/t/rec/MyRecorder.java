...
class MyRecorder {
    ...
    Thread recordThread = new Thread(new Runnable() {
        ...
        public void run() {
            ...
            try {
                //开通输出流到指定的文件
                BufferedOutputStream bos_rec = new BufferedOutputStream(new FileOutputStream(audioFile_rec));
                BufferedOutputStream bos_echo = new BufferedOutputStream(new FileOutputStream(audioFile_echo));
                //添加_立体声_rain
                BufferedOutputStream bos_stereo = new BufferedOutputStream(new FileOutputStream(audioFile_stereo));
                ...
                RandomAccessFile raf_rec = new RandomAccessFile(audioFile_rec,"rw");
                raf_rec.seek(0);
                raf_rec.write(whead);
                raf_rec.close();

                RandomAccessFile raf_echo = new RandomAccessFile(audioFile_echo,"rw");
                raf_echo.seek(0);
                raf_echo.write(whead);
                raf_echo.close();

                RandomAccessFile raf_stereo = new RandomAccessFile(audioFile_stereo,"rw");
                raf_stereo.seek(0);
